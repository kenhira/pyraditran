...
! Description:  Modified from DISORT.f originally distributed in the DISORT 
!               package.
! ==============================================================================
...
!     ** Calculate cumulative optical depth and dither single-scatter albedo
...
!     ** Handle special case for getting albedo and transmissivity of medium for
...
!       ** Get normalized associated Legendre polys.  with negative arguments
...
!         ** Solve eigenfunction problem in Eq. STWJ(8B), STWL(23f); return
...
!         ** Version 3: upgraded subroutine UPBEAM to use reduced order matrix
!         ** and LAPACK.
          IF( FBEAM.GT.0.0 ) THEN
...
!       ** Solve for constants of integration in homogeneous solution (general
!       ** boundary conditions).
...
!         ** Print azimuthally averaged intensities at user angles
  ...
  140       CONTINUE
        ...
        ELSE
!         ** Increment intensity by current azimuthal component (Fourier cosine
...
!                     (DISORT3 paper Eq. (43), third term on the righ hand side)
!          
...
!         ELSE IF ( DELTAM .OR. PMOM(NSTR,LC) .EQ. PMOM(NSTR+1,LC) ) THEN
...
!          if(PMOM(NSTR,LC).gt.PMOM(NSTR+1,LC)) then ! fix to avoid negative sigma_sq
             f = pmom(nstr, lc)
...
!           PRINT*, F, C, sigma_sq
!          else
!            f = PMOM( NSTR, LC ) ! go back to delta-M when PMOM(NSTR,LC) < PMOM(NSTR+1,LC)
!          endif  
...
!C
!Cc                               ** Shift positive user angle cosines to
...
!        special banded-matrix format required by LAPACK/LINPACK routines
!
!
!    I N P U T      V A R I A B L E S:
!
