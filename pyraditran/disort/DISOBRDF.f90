! ==============================================================================
! DISOBRDF.f90
! Author: Ken Hirata, ken.hirata@colorado.edu
! Description:  Modified from DISOBRDF.f originally distributed in the DISORT 
!               package.
! ==============================================================================
...
!
!  ONLYFL                      Flag that decides whether to only output flux
!
!  NUMU                        Number of user polar angles
!
!  RHOQ(MI,0:MI,0:NAZZ)        Quadrature fourier expanded BRDF: rho^m * PI 
!                              1st index: output polar angle 
!                              2nd index: input polar angle, 0 is direct beam
!                              3rd index: azimuth index
!
!  RHOU(NUMU,0:MI,0:NAZZ)     User defined fourier expanded BRDF: rho^m * PI
!                              same index as RHOQ
!
!  EMUST(NUMU)                Directional emissivity at user angles
!
!  BEMST(MI)                   Directional emissivity at quadrature angles
!
!
! +-------------------------------------------------------------------+
...
!     ** Calculate computational polar angle cosines and associated quadrature
...
!     ** Preparation before Fourier loop: COSMP GMU, GWT
      ...
      DO 10 K = 1, NMUG/2
        ...
        IF(.NOT.LAMBER .AND. .NOT.ONLYFL) THEN
          ...
          ENDDO
        ENDIF
!       Initialize zeroth order separately to avoid random error when ONLYFL = .TRUE. 
!       11-27-2017         
!       IF(.NOT.LAMBER .AND. .NOT.ONLYFL) THEN
!         DO J = 0, NSTR-1
!           COSMP(J,K) = COS(J*PI*GMU(K))
...
!     ** Version 3
!     ** Compute analytic surface bidirectional reflectance at user angles
      IF(.NOT. LAMBER) THEN
        DO 190 IU = 1, NUMU
          DO 200 J = 1, NPHI
            IF(UMU(IU) .LE. 0.) THEN
...
!     ..
      ...
      IF(MAZIM .GT. 2*NN) THEN
        CALL ERRMSG( 'MXSTR TOO LOW - INCREASE MXSTR OR DECREASE NSTR', .TRUE. )
      ENDIF
...
!       ** For 0th azimuth component, integrate bidirectional reflectivity at
!       ** reflection polar angle cosines -CMU- and incident angle cosines -GMU-
!       ** to get directional emissivity at computational angle cosines -CMU-.
        IF(MAZIM .EQ. 0) THEN
...
!             ** For 0th azimuth component, integrate bidirectional reflectivity
!             ** at reflection angle cosines -UMU- and incident angle cosines
!             ** -GMU- to get directional emissivity at user angle cosines
!             ** -UMU-.
              IF(MAZIM .EQ. 0) THEN
                DREF  = 0.0
                DO 160 JG = 1, NMUG
